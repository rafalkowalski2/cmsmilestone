				...
				<div class="form-group">
  					...
  					<select class="form-control" name="parent_folder" id="parent_folder" hidden>
    					...
    					<?php
							...
							{
								if(key_exists($key, $levels))
								{
									if($levels[$key] > 1)
									{
										
										show($value, $key);
										/*echo' <option value="'.$value['id'].'" data-value="'.$value['name'].'">'.$value['display_name'].'</option>';
										foreach($value['children'] as $key2 => $value2)
										{
											echo' <option value="'.$value2['id'].'" data-value="'.$value2['name'].'">--'.$value2['display_name'].'</option>';
											if(is_array($value2['children']))
											{
												foreach($value2['children'] as $key3 => $value3)
												{
													echo' <option value="'.$value3['id'].'" data-value="'.$value3['name'].'">----'.$value3['display_name'].'</option>';
												}
											}
										}
									}
